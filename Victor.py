...
def get_poc_data(df_slice, bins):
    p_min, p_max = float(df_slice['Low'].min()), float(df_slice['High'].max())
...
if raw_df is not None and not raw_df.empty:
    ...
    with c2:
        ...
        st.write(f"📈 趨勢防線：**{'🔥 多頭控盤' if price_now > curr[sma20_col] else '❄️ 空頭盤整'}**")
    with c3:
        rsi_col = 'RSI_14' if 'RSI_14' in df_d.columns else df_d.filter(like='RSI').columns[0]
        score = sum([30 if price_now > curr[sma20_col] else 0, 20 if curr[rsi_col] > 50 else 0, 50 if price_now > poc_price else 0])
    ...
    with tab_tech:
        ...
        if bbu_col and bbl_col:
            fig.add_trace(go.Scatter(x=df.index, y=df[bbu_col], name="布林上軌", line=dict(color='rgba(128, 128, 128, 0.5)', width=1.5, dash='dash')), row=1, col=1)
            fig.add_trace(go.Scatter(x=df.index, y=df[bbl_col], name="布林下軌", line=dict(color='rgba(128, 128, 128, 0.5)', width=1.5, dash='dash')), row=1, col=1)
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df[rsi_col], name="RSI", line=dict(color='#9467bd')), row=3, col=1)
        fig.add_hline(y=70, line_dash="dash", line_color="red", row=3, col=1)
        fig.add_hline(y=30, line_dash="dash", line_color="green", row=3, col=1)
        
        macd_hist_col = df_d.filter(like='MACDh').columns[0] if len(df_d.filter(like='MACDh').columns) > 0 else 'MACD'
        fig.add_trace(go.Bar(x=df.index, y=df[macd_hist_col], name="MACD柱"), row=4, col=1)
        
        mfi_col = df_d.filter(like='MFI').columns[0] if len(df_d.filter(like='MFI').columns) > 0 else 'MFI'
        fig.add_trace(go.Scatter(x=df.index, y=df[mfi_col], name="MFI", fill='tozeroy', line=dict(color='#17becf')), row=5, col=1)
    ...
    with tab_chip:
        ...
        with col_c2:
            ...
            with st.container(border=True):
                ...
                st.write(f"🔥 **資金活躍度 (MFI)：** {curr[mfi_col]:.1f}")
    ...
    with tab_radar:
        ...
        with col_r1:
            radar_vals = [
                100 if price_now > curr[sma20_col] else 20, 
                curr[rsi_col], 
                curr[mfi_col], 
                100 if curr[macd_hist_col] > 0 else 20, 
                100 if curr[obv_col] > df[obv_col].shift(5).iloc[-1] else 30, 
                100 if curr['Net_Flow'] > 0 else 30
            ]
        ...
        with col_r2:
            ...
            with st.container(border=True):
                ...
                st.write(f"⚡ **RSI 動能 ({curr[rsi_col]:.1f})**")
                st.write(f"🚥 **MACD 趨勢 ({curr[macd_hist_col]:.2f})**")
            ...
            with st.container(border=True):
                ...
                if price_now > ref_20_val:
                    if future_ref < ref_20_val:
                        st.success("現價成功壓制扣抵，且未來扣抵持續下沉！20MA上升動能深度極強，此處回踩將化為極佳暴風加碼點。")
                    else:
                        st.info("目前佔據扣低優勢，惟未來扣抵緩步攀升。短期多頭慣性仍在，預期走勢維持震盪盤堅，不宜過度追高。")
                else:
                    if future_ref > ref_20_val:
                        st.error("現價落入扣高危機，未來扣抵陡峭夾擊。均線下彎地心引力沉重，走勢恐加速探底，切勿盲目進場接刀。")
                    else:
    ...
    with tab_scan:
        ...
        if st.session_state.is_scanning:
            ...
            for i in range(idx, end_idx):
                ...
                if sdf is not None and len(sdf) > 60:
                    ...
                    s_sma20 = 'SMA_20' if 'SMA_20' in sdf.columns else sdf.filter(like='SMA').columns[0]
                    s_rsi = 'RSI_14' if 'RSI_14' in sdf.columns else sdf.filter(like='RSI').columns[0]
                    
                    sdf['Bias_20'] = ((sdf['Close'] - sdf[s_sma20]) / sdf[s_sma20]) * 100
                    sdf['Net_Flow'] = (sdf['Close'].diff() * sdf['Volume']).fillna(0)
                    ...
                    if poc_bias >= 0 and poc_bias <= 3.0 and t_val >= 1.2: status_tag = "⚡ 剛衝鋒"
                    elif abs(poc_bias) <= 1.0 and t_val < 1.2: status_tag = "🛡️ 守成本"
                    elif poc_bias > 3.0: status_tag = "🚀 鷹擊長空"
                    else: status_tag = "⚠️ 潛水艇"
                    ...
                    if sdf['Net_Flow'].iloc[-1] > 0:
                        ...
                        if count_w >= lookback_val:
                            st.session_state.scan_results.append({
                                ...
                                "60日資金流量柱": sdf['Net_Flow'].tail(60).tolist(), "RSI": round(float(sdf[s_rsi].iloc[-1]), 1),
        ...
        if st.session_state.scan_results:
            ...
            with f6: poc_range = st.slider("POC乖離率篩選範圍 (%)", min_value=-200.0, max_value=200.0, value=(-200.0, 200.0), step=0.5)
            with f7: selected_tags = st.multiselect("過濾籌碼戰略位階", options=["⚡ 剛衝鋒", "🛡️ 守成本", "🚀 鷹擊長空", "⚠️ 潛水艇"], default=["⚡ 剛衝鋒", "🛡️ 守成本", "🚀 鷹擊長空", "⚠️ 潛水艇"])
